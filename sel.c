...
/*	fprintf(cgiOut, "<head><meta charset=\"utf-8\"/><title>查询结果</title>\
	...
	if (name[0] == '*')
	{
		sprintf(sql, "select * from student,grade,school where student.id = grade.id and school.cno = grade.cno and student.sta=1");
	}
	else
	{
		sprintf(sql, "select * from student,grade,school where name = '%s' and student.id = grade.id and school.cno = grade.cno and student.sta=1", name);
	}
